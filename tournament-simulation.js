...
(function(){
  ...
  function cleanName(s){return s.replace(/\(test\)/gi,'').replace(/^\s*\d+\.\s*/,'').trim()}
  ...
  function render(){
    ...
    $('liveGroups').innerHTML=groups.map((players,gi)=>{
      ...
      ${rounds.map((round,ri)=>`<div class="round-block"><div class="round-title">Runde ${ri+1}</div>${round.map(([a,b])=>`<div class="match-row simulation-match" role="button" tabindex="0" data-player-a="${a}" data-player-b="${b}"><div class="match-players"><strong>${a}</strong><span class="muted">vs</span><strong>${b}</strong><span class="match-state">Klar</span></div></div>`).join('')}</div>`).join('')}</div>`
    }).join('');
    $('groupLobby').scrollIntoView({behavior:'smooth',block:'start'});
  }
  document.addEventListener('click',e=>{
    const b=e.target.closest('#startGroupsBtn');
    if(b&&b.textContent.toLowerCase().includes('simulering')){e.preventDefault();e.stopImmediatePropagation();render();return}
    const m=e.target.closest('.simulation-match');
    if(m){alert(`${m.dataset.playerA} vs ${m.dataset.playerB}\n\nTestkamp. Neste steg kobler denne raden til spillerom / tilskuervisning.`)}
  },true);
})();