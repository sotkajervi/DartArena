...
async function boot(){
 ...
 function canCheckout(score,darts){if(darts===1)return doubles.includes(score);if(darts===2)return all.some(a=>doubles.includes(score-a));if(darts===3)return all.some(a=>all.some(b=>doubles.includes(score-a-b)));return false}
 ...
 ensureDialog();
 function askDarts(score){return new Promise(resolve=>{const dlg=$('checkoutDartsDialog'),buttons=$('checkoutDartsButtons'),allowed=possibleDarts(score);let done=false;const finish=n=>{if(done)return;done=true;document.removeEventListener('keydown',keyHandler,true);dlg.classList.add('hidden');resolve(n)};const keyHandler=e=>{if(dlg.classList.contains('hidden'))return;if(['1','2','3'].includes(e.key)){e.preventDefault();e.stopImmediatePropagation();const n=Number(e.key);if(allowed.includes(n))finish(n)}else if(e.key==='Escape'){e.preventDefault();e.stopImmediatePropagation();finish(null)}};$('checkoutDartsTitle').textContent=`Checkout ${score}`;$('checkoutDartsHint').textContent='Hvor mange piler brukte du på å lukke? Trykk 1, 2 eller 3.';buttons.innerHTML='';[1,2,3].forEach(n=>{const b=document.createElement('button');b.type='button';b.className=allowed.includes(n)?'primary':'outline';b.textContent=String(n);b.disabled=!allowed.includes(n);b.onclick=()=>finish(n);buttons.appendChild(b)});$('checkoutDartsCancel').onclick=()=>finish(null);document.addEventListener('keydown',keyHandler,true);dlg.classList.remove('hidden')})}
 async function finishCheckout(score,darts){
   ...
   try{
    ...
    if(match.match_mode!=='sets'){patch[legKey]=newLegs;if(newLegs>=legsNeeded){patch[scoreKey]=0;patch.status='finished';patch.winner_id=me;patch.finished_at=new Date().toISOString()}else{const nextLeg=Number(match.current_leg||1)+1;patch.player1_score=match.game;patch.player2_score=match.game;patch.current_leg=nextLeg;patch.turn_player_id=starterFor(1,nextLeg)}}
    else if(newLegs>=legsNeeded){const newSets=Number(match[setKey]||0)+1,setsNeeded=Math.floor(Number(match.best_of_sets)/2)+1;patch[setKey]=newSets;if(newSets>=setsNeeded){patch[legKey]=newLegs;patch[scoreKey]=0;patch.status='finished';patch.winner_id=me;patch.finished_at=new Date().toISOString()}else{const nextSet=Number(match.current_set||1)+1;patch.player1_legs=0;patch.player2_legs=0;patch.player1_score=match.game;patch.player2_score=match.game;patch.current_set=nextSet;patch.current_leg=1;patch.turn_player_id=starterFor(nextSet,1)}}
 ...
 async function intercept(){if(busy||match.status!=='playing'||match.turn_player_id!==me)return false;const input=$('matchScoreInput'),n=Number(input.value);if(!Number.isInteger(n)||n<0||n>180)return false;const scoreKey=me===match.player1_id?'player1_score':'player2_score',remaining=Number(match[scoreKey]);if(n!==remaining)return false;const allowed=possibleDarts(remaining);if(!allowed.length){$('matchMessage').textContent=`${remaining} kan ikke avsluttes på tre piler.`;return true}const darts=await askDarts(remaining);if(darts)await finishCheckout(remaining,darts);return true}
