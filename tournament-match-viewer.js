const URL = 'https://jqpxlbhwvskhjbqrbidk.supabase.co';
...
function render() {
  ...
  const statusText = {
    live: 'Kampen pågår live',
    finished: 'Kampen er ferdig',
    wo: 'Kampen er avgjort på WO',
    pending: 'Kampen er ikke startet ennå',
  };
...
function renderPresence() {
  ...
  const viewers = Object.values(presence.presenceState())
    .flat()
...
async function boot() {
  ...
  if (error || !data) {
    console.error('Kunne ikke laste turneringskamp:', error);
    location.replace('./');
    return;
  }
  ...
  if (profileError) {
    console.error('Kunne ikke laste spillernavn:', profileError);
  }
  ...
  presence = db.channel(`tournament-match-viewers-${matchId}`, {
    config: {
...
window.addEventListener('pagehide', () => {
  try {
    ...
    console.debug('Presence cleanup skipped:', error);
  }
});

boot().catch((error) => {
  console.error('Tilskuervisningen kunne ikke starte:', error);
});
